...
class SSearch():
    ...
    def run(self, train, test, scaler, opt, iter_id, fold_id):
        ...
        for param_id, params in enumerate(ParameterGrid(self.params_grid)):
            ...
            if fold_id > 0:
                filename = f'{opt.estimation_model}_{opt.base_model}_iter{iter_id}_fold{fold_id}_param{param_id}.csv'
            else:
                filename = f'{opt.estimation_model}_{opt.base_model}_iter{iter_id}_param{param_id}.csv'
    ...
    def get_params_info(self):
        ...
        df_main = df_ids.merge(df_params)
